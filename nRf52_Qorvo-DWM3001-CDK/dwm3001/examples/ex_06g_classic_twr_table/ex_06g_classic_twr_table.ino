...
#define APP_NAME "Classic SS TWR Table"
...
static void print_table_header_once()
{
...
static void print_tof_and_range(double tof_seconds, double distance_meters)
{
    print_table_header_once();

    const uint32_t tof_microseconds = (uint32_t)((tof_seconds * 1e6) + 0.5);
    long meters_part = (long)distance_meters;
    long centimeters_part = (long)((distance_meters - meters_part) * 100.0 + 0.5);
    ...
    char row[64];
    snprintf(row, sizeof(row), "%lu\t%ld.%02ld\r\n", (unsigned long)tof_microseconds, meters_part, centimeters_part);
    UART_puts(row);
}
        ...
        if (status_reg & SYS_STATUS_RXFCG_BIT_MASK)
        {
            ...
            if (frame_len <= sizeof(rx_buffer))
            {
                ...
                if (memcmp(rx_buffer, rx_resp_msg, ALL_MSG_COMMON_LEN) == 0)
                {
                    ...
                    /* Push the ToF/range values out as a two-column table for easier logging. */
                    print_tof_and_range(tof, distance);
                }
            }
        }
        else
