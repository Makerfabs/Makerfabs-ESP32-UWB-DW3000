...
/* Node addressing (16-bit short addresses). Update these to match your nodes. */
#define RESPONDER_ID 0xCA94
...
void setup() {
  ...
  if(dwt_configure(&config)) // if the dwt_configure returns DWT_ERROR either the PLL or RX calibration has failed the host should reset the device
  {
    ...
    /* Set PAN ID and short addresses in the frames (little-endian). */
    tx_poll_msg[3] = 0xCA;
    tx_poll_msg[4] = 0xDE;
    tx_poll_msg[5] = (uint8_t)(RESPONDER_ID & 0xFF);
    tx_poll_msg[6] = (uint8_t)(RESPONDER_ID >> 8);
    /* Local sender ID fixed to 0x5E16. */
    tx_poll_msg[7] = 0x16;
    tx_poll_msg[8] = 0x5E;
    ...
    rx_resp_msg[5] = 0x16;
    rx_resp_msg[6] = 0x5E;
    rx_resp_msg[7] = (uint8_t)(RESPONDER_ID & 0xFF);
    rx_resp_msg[8] = (uint8_t)(RESPONDER_ID >> 8);
        ...
        if (status_reg & SYS_STATUS_RXFCG_BIT_MASK)
        {
            ...
            if (frame_len <= sizeof(rx_buffer))
            {
                ...
                if (memcmp(rx_buffer, rx_resp_msg, ALL_MSG_COMMON_LEN) == 0)
                {
                    ...
                    /* Display computed distance using integer formatting (no float printf support on nRF52). */
                    ...
                    snprintf(json_msg,
                             sizeof(json_msg),
                             "{\"t1\":%lu,\"t2\":%lu,\"t3\":%lu,\"t4\":%lu,\"distance_m\":%ld.%02ld}",
                             ...
                             meters_part,
                             centimeters_part);
                    test_run_info((unsigned char *)json_msg);
                }
            }
        }
        else
