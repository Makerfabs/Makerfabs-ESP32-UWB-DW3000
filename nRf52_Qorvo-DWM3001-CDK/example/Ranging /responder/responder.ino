...
void setup() {
  ...
  if(dwt_configure(&config)) // if the dwt_configure returns DWT_ERROR either the PLL or RX calibration has failed the host should reset the device
  {
    ...
    rx_poll_msg[3] = 0xCA;
    rx_poll_msg[4] = 0xDE;
    /* Local receiver ID fixed to 0xCA94. */
    rx_poll_msg[5] = 0x94;
    rx_poll_msg[6] = 0xCA;
    rx_poll_msg[7] = (uint8_t)(INITIATOR_ID & 0xFF);
    rx_poll_msg[8] = (uint8_t)(INITIATOR_ID >> 8);

    tx_resp_msg[3] = 0xCA;
    tx_resp_msg[4] = 0xDE;
    tx_resp_msg[5] = (uint8_t)(INITIATOR_ID & 0xFF);
    tx_resp_msg[6] = (uint8_t)(INITIATOR_ID >> 8);
    /* Local receiver ID fixed to 0xCA94. */
    tx_resp_msg[7] = 0x94;
    tx_resp_msg[8] = 0xCA;
